...
if __name__ == '__main__':
  ...
  for idx,row in codesearch_df.iterrows():
    ...
    pred_doc = row['GPT-4 documentation']
    pred_doc = pred_doc.lstrip().rstrip()
    pred_doc = pred_doc.split(".")[0] + "."
    doc_id = str(idx+1)
    references = []
    predictions = []
    references.append(doc_id+'\t'+ref_doc)
    predictions.append(doc_id+'\t'+pred_doc)
    (goldMap, predictionMap) = computeMaps(predictions, references) 
    bleu_list.append(bleuFromMaps(goldMap, predictionMap)[0])
